    ...
    <div class="container-dashboard">
        <div class="sidenav-bar">
            <ul>
                ...
                <li><a href="{{ route('quiz.quizIndex', $id) }}">Activities</a></li>
                <li><button id="openPupilsModal">Pupils</button></li>
       ...
       <div class="my-classes">
        ...
        <hr />
            ...
            <!-- Modal for add lesson -->
                <div id="pupilsModal" class="modal">
                    <div class="pupils-modal-content">
                        <p id="closePupilsModal" class="close">&times;</p>
                        <!-- Loop through assigned pupils and display their details -->
                        <div class="pupils-class-list">
                            <div class="modal-pupils-title">
                                <h3> Class Pupils </h3>
                            </div>
                            <div class="adjust-list">
                                @foreach($assignedPupils as $pupil)
                                    <p class="pupils-users-list">{{ optional($pupil->user)->name }}</p>
                                    <hr />
                 ...
                 @if(isset($savedLessons) && !empty($savedLessons))  
                    @foreach($savedLessons as $savedLesson)
                    <div class="quarter-class">
                        <div class="option-list">
                            <a class="add-lesson-edt" href="{{ route('assign_class.editLesson', $savedLesson->id) }}"><i class="fa-regular fa-pen-to-square"></i></a>
                            <a class="add-lesson-delete" href="{{ route('assign_class.deleteLesson', $savedLesson->id) }}" onclick="return confirm('Are you sure you want to delete this lesson?')">&times;</a>
                        </div>
                        <br />
                        <div class="quarterly-lesson">
                            <p class="add-lesson-date">{{ $savedLesson->created_at }}</p>
                            <p class="add-lesson-title">{{ $savedLesson->quarter }}</p>
                            <p class="add-lesson-content">{{  $savedLesson->topic }}</p>
                            <br />
                        </div>
        ...
        <form action="{{ route('assign_class.postLesson')}}" method="POST">
            @csrf
            <div>
