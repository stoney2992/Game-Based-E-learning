...
use App\Http\Controllers\Api\QuizController as ApiAssignClassController;
...
public function mylessonPupils($class_id)
{
    ...
    // Get lessons related to the class
    $lessonData = $classData->lessons;

    // If you want to include lessons for a specific user (teacher or pupil), you can modify the query
    // $lessonData = AddLesson::where('class_id', $class_id)->where('user_id', Auth::user()->id)->get();

    return view('assign_class.mylessonPupils', compact('classData', 'lessonData'));
}
...
public function postLesson(Request $request)
{
    ...
    $add_lesson->topic =nl2br($request->topic);
...
{
    // Get all assigned classes for pupils
    $assignedClasses = AssignClassPupil::getRecord();

    // Group assigned classes by class_id
    $groupedClasses = $assignedClasses->groupBy('class_id');

    // Get assigned teachers for each class
    $assignedTeachers = AssignClassModel::whereIn('class_id', $groupedClasses->keys())
        ->get()
        ->keyBy('class_id');
...
{
    // Retrieve quizzes for the specified class
